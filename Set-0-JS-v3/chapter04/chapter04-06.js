...
// 4.6) Overloaded distance function
//     - throw errors instead of strings. Errors have useful stack trace data
 ...
 function distance (){
     try{
        if (typeof (arguments[0]) == 'number'){
            if(arguments.length != 4){
                throw 'error1';     // This error code would mean: "Insufficient parameters"
            }
            else{
                if (arguments.length==4){
                    var x1=arguments[0];
     ...
     return tot
        ...
        return err       // attending the feedback comment, I decided to return an error code, so the calling code can know 
